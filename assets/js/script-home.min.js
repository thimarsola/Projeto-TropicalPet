...
item.preloaded=!0}}});var RETINA_NS='retina';$.magnificPopup.registerModule(RETINA_NS,{options:{replaceSrc:function(item){return item.src.replace(/\.\w+$/,function(m){return'@2x'+m})},ratio:1},proto:{initRetina:function(){if(window.devicePixelRatio>1){var st=mfp.st.retina,ratio=st.ratio;ratio=!isNaN(ratio)?ratio:ratio();if(ratio>1){_mfpOn('ImageHasSize'+'.'+RETINA_NS,function(e,item){item.img.css({'max-width':item.img[0].naturalWidth/ratio,'width':'100%'})});_mfpOn('ElementParse'+'.'+RETINA_NS,function(e,item){item.src=st.replaceSrc(item,ratio)})}}}}});_checkInstance()}));$(function(){$('.gallery__row').magnificPopup({delegate:'a',type:'image',gallery:{enabled:!0}})});$(function(){const speed=1000;$('.faq__row__container__content__item__header').on("click",function(){$('.faq__row__container__content__item__body').stop().slideUp(speed);$(this).parent().find('.faq__row__container__content__item__body').slideToggle();$('.faq__row__container__content__item__header span').text('+');$(this).children('.faq__row__container__content__item__header span').text('-')})});/*!
 * sweetalert2 v7.0.8
 * Released under the MIT License.
 */
...
config.trigger=target;result.push(ScrollTrigger.create(config))});return result};ScrollTrigger.sort=function(func){return _triggers.sort(func||function(a,b){return(a.vars.refreshPriority||0)*-1e6+a.start-(b.start+(b.vars.refreshPriority||0)*-1e6)})};_getGSAP()&&gsap.registerPlugin(ScrollTrigger);exports.ScrollTrigger=ScrollTrigger;exports.default=ScrollTrigger;Object.defineProperty(exports,'__esModule',{value:!0})})));let hero=gsap.timeline({scrollTrigger:{trigger:'.hero'}});let about=gsap.timeline({scrollTrigger:{trigger:'.about',start:'left bottom'}});let products=gsap.timeline({scrollTrigger:{trigger:'.products',start:'left center'}});let pillar=gsap.timeline({scrollTrigger:{trigger:'.pillar',start:'left center'}});let clinic=gsap.timeline({scrollTrigger:{trigger:'.clinic',start:'left bottom'}});let gallery=gsap.timeline({scrollTrigger:{trigger:'.gallery',start:'left bottom'}});let testimonial=gsap.timeline({scrollTrigger:{trigger:'.testimonial',start:'left bottom'}});let faq=gsap.timeline({scrollTrigger:{trigger:'.faq',start:'left bottom'}});let contact=gsap.timeline({scrollTrigger:{trigger:'.contact',start:'left bottom'}});hero.from('.hero__content__header',{opacity:0,duration:1,x:150,ease:'Back.easeInOut'}).from('.hero__content__container',{opacity:0,duration:1,x:150,ease:'Power4.easeInOut'});about.from('.about__row__container',{opacity:0,duration:1,y:150,ease:'Back.easeInOut'}).from('.about__row__image',{opacity:0,duration:1,x:-150,ease:'Power4.easeInOut'});products.from('.products',{opacity:0,duration:1,y:150,ease:'Expo.easeInOut'});pillar.from('.pillar__container__header',{opacity:0,duration:1,x:150,ease:'Expo.easeInOut'}).from('.pillar__container__content',{opacity:0,duration:1.5,x:150,ease:'Expo.easeInOut'});clinic.from('.clinic__row__container',{opacity:0,duration:1,y:150,ease:'Back.easeInOut'}).from('.clinic__row__image',{opacity:0,duration:1,x:-150,ease:'Power4.easeInOut'});gallery.from('.gallery',{opacity:0,duration:1.5,y:150,ease:'Back.easeInOut'});testimonial.from('.testimonial',{opacity:0,duration:1,ease:'Bounce.easeInOut'});faq.from('.faq__row__container',{opacity:0,duration:2,x:-150,ease:'Bounce.easeInOut'});contact.from('.contact',{opacity:0,duration:1.5,y:50,ease:'Bounce.easeInOut'});let footer=gsap.timeline({scrollTrigger:{trigger:'.footer',start:'left bottom'}});gsap.from('.header',{opacity:0,duration:1.5,delay:.5,y:-50,ease:'Bouce.easeInOut'});gsap.from('.whatsapp',{opacity:0,duration:1.5,delay:1.5,x:0,ease:'Bouce.easeInOut'});footer.from('.footer',{opacity:0,duration:1.5,y:100,ease:'Bouce.easeInOut',})